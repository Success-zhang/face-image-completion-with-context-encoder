...
from MyImageLoader import MyDataset
import numpy as np
...
parser.add_argument('--dataroot',  default='dataset/train/celeba_train', help='path to dataset')
...
# txt = "./dataset/celeba_train.txt"
txt = "./dataset/celeba_train.txt"
dataset = MyDataset(txt=txt, transform=transforms.Compose([
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f / %.4f l_D(x): %.4f l_D(G(z)): %.4f'
              % (epoch, opt.niter, i, len(dataloader),
                 errD.data.item(), errG_D.data.item(), errG_l2.data.item(), D_x, D_G_z1,))
        if i % 100 == 0:
